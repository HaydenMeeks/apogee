...
export default function OverviewTab({ plan, completions }) {
  ...
  wkData.forEach((d,i)=>{
    ...
    if(d.hasRace){
      ...
      pts+=`<text x="${cx}" y="${cy-sz-4}" text-anchor="middle" font-family="DM Mono,monospace" font-size="8.5" fill="#EF4444" font-weight="600">${d.km}</text>`;
    } else if(d.hasAet){
      const sz=5;
    ...
    if(d.km>=maxKm*0.87||d.isCur||i===0)
      pts+=`<text x="${cx}" y="${cy-9}" text-anchor="middle" font-family="DM Mono,monospace" font-size="9" fill="${d.isCur?'#00C46A':'rgba(244,244,242,0.5)'}" font-weight="${d.isCur?700:400}">${d.km}</text>`;
  });
